...
async function main() {
  ...
  if (isWSL || !hasDisplay) {
    ...
    log(
      "   Firebase CLI commands may hang when trying to open a browser.",
      "yellow",
    );
  ...
  if (existsSync(firebaseJsonPath) && existsSync(firebaseRCPath)) {
    ...
    log(
      "\n   ℹ️  Note: If you need to use Firebase CLI commands later,",
      "yellow",
    );
...
`;
  ...
  if (!existsSync(rulesPath)) {
    ...
    if (
      !existingRules.includes("match /notes/") ||
      !existingRules.includes("match /assignments/")
    ) {
      log(
        "⚠️  firestore.rules exists but may be missing required rules.",
        "yellow",
      );
      log(
        "   Consider updating it with the rules for notes and assignments collections.",
        "yellow",
      );
    } else {
  ...
  if (existsSync(firebaseJsonPath)) {
    try {
      ...
      log(
        "⚠️  Could not update firebase.json (this is okay if Firestore is already configured)",
        "yellow",
      );
    }
  }
  ...
  if (existsSync(envPath)) {
    ...
    if (hasAllVars) {
      log(
        "✅ .env file appears to have all required Firebase configuration variables",
        "green",
      );
    } else {
      log(
        "⚠️  .env file exists but may be missing some Firebase configuration variables",
        "yellow",
      );
...
`;
  ...
  log(
    "1. Link to your existing Firebase project (if not already linked):",
    "blue",
  );
  ...
  log(
    "   - Click web icon (</>) to add a web app (if not already added)",
    "yellow",
  );
